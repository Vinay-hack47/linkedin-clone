...
const SocialOptions = ({ post }: { post: IPostDocument }) => {
  ...
  return (
    <div>
        <div className="flex flex-1 mx-2 p-2 justify-between">
         <p>
         {
          ...
          <p> 
          {
            (post.comments && post.comments.length >0) && (<p onClick={() =>setCommentOpen(!commentOpen)} className="text-xm text-gray-500 hover:text-blue-500 hover:underline hover:cursor-pointer">{post.comments.length} comments</p>)
          }
          </p>
        </div>
      <div className="flex items-center m-1 justify-between">
        <Button
          ...
          variant={"ghost"}
          className="flex items-center gap-1 rounded-lg text-gray-600 hover:text-black cursor-pointer"
        >
          <ThumbsUp className={`${liked && 'fill-[#378FE9]'}`}></ThumbsUp>
          <p className="className={`${liked && 'text-[#378FE9]'}`}">Like</p>
        </Button>
        <Button
